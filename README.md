...
Underneath it was a delivery gap worth more than the code: **no guest image had been published since 2026-04-21.** `release.yml` was correctly wired the whole time — it builds the CLI and the image together and publishes both on a `lumina-v*` tag — but no tag was cut for three months, so fixes landed in git and reached nobody. A stale image now announces itself on stderr with the command to fix it, and `ImagePuller.fallbackTag` tracks the current release.

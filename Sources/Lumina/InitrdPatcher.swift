...
enum InitrdPatcher {
    ...
    private static func customInitScript() -> String {
        ...
        lines += [
            ...
            "done",
            "",
            "# Static networking for VZ NAT (AF_PACKET/DHCP not available)",
            "# VZNATNetworkDeviceAttachment uses vmnet shared mode which defaults to",
            "# 192.168.64.0/24 with gateway at 192.168.64.1. Allow override via kernel",
            "# command line: lumina.gw=<ip> lumina.ip=<ip/mask>",
            "LUMINA_GW=192.168.64.1",
            "LUMINA_IP=192.168.64.4/24",
            "for param in $(cat /proc/cmdline); do",
            "  case \"$param\" in",
            "    lumina.gw=*) LUMINA_GW=\"${param#lumina.gw=}\" ;;",
            "    lumina.ip=*) LUMINA_IP=\"${param#lumina.ip=}\" ;;",
            "  esac",
            "done",
            "",
            "ip addr add \"$LUMINA_IP\" dev eth0 2>/dev/null",
            "ip route add default via \"$LUMINA_GW\" dev eth0 2>/dev/null",
            "",
            ...
            "",
            "# DNS — VZNATNetworkDeviceAttachment runs InternetSharing which starts",
            "# named(8) in caching-only mode on the gateway. The gateway is the ONLY",
            "# reliable nameserver — UDP to external resolvers (8.8.8.8, 1.1.1.1) is",
            "# dropped by the vmnet PF-based NAT. Including unreachable fallback",
            "# servers causes c-ares (used by curl) to wait for timeouts, adding",
